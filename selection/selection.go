...
func Draw(selected *int, options *[]string, opts wtopts.Opts) []string {
	var strs []string
...
func HandleKey(selected *int, options *[]string, key string, opts wtopts.Opts) error {
	if key == "Down" || key == "j" {
		*selected++
...
func Selection(options []string, customOpts ...wtopts.Opts) (result int, err error) {
